...
class SecondDraftAssigner {
  static assign(groups, highestIndex, assignedBabies, currentSeed) {
      ...
      for (const group of groups) {
        ...
        if (unassignedMembers.length > 0) {
          ...
          try {
            ...
            throw new Error(`[SecondDraftAssigner] Failed to assign baby to ${member.getName()}: ${error.message}`);
          }
        }
      }
    ...
    console.log(`[SecondDraftAssigner] ✓ All members assigned in second draft`);
    return assignedBabies;
  }
}

module.exports = SecondDraftAssigner;
